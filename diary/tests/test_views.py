from dajngo.contrib.auth import get_user_model
...
class TestdiaryCreateView(LoggedInTestCase):
    ...
    def test_create_diary_seccess(self):
        """日記作成処理が成功することを検証する"""
...
class TestDiaryUpdateView(LoggedInTestCase):
    ...
    def test_update_diary_success(self):
        ...
        # 日記詳細ページへのリダイレクトを検証
        self.assertRedirects(response, reverse_lazy('diary:diary?detail', kwargs={'pk': diary.pk}))
...
class TestDiaryDeleteView(LoggedInTestCase):
    ...
    def test_delete_diary_success(self):
        ...
        # 日記データが削除されたかを検証
        self.aasertEqual(Diary.objects.filter(pk=diary.pk).count(), 0)
